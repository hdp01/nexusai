...
export const PricingWrapper: React.FC<{
  ...
  className?: string
  waveColor?: string
}> = ({ children, contactHref, className, type = 'waves', waveColor = "text-neutral-900/10" }) => (
  <article
    className={cn(
      'min-h-[300px] h-[600px] max-h-[500px] max-w-sm w-full bg-purple-500 relative overflow-hidden rounded-3xl text-white shadow-2xl transition-transform hover:scale-[1.02] duration-300',
      className
    )}
  >
    <span
      className={
        'w-full h-full absolute top-0 left-0 z-[2] p-8 flex flex-col items-start justify-start sm:gap-10 gap-7'
      }
    >
      {children}
      <div className={'w-full h-full flex items-end justify-end text-base'}>
        <Link href={contactHref} className={'w-full h-fit'}>
          <button className={'h-14 w-full bg-white/90 hover:bg-white backdrop-blur-sm rounded-xl text-neutral-900 font-bold transition-all shadow-lg'}>
            Get Started
          </button>
        </Link>
      </div>
    ...
    {type === 'waves' && (
      <>
        <div className={cn('w-fit h-fit absolute -top-[106px] sm:left-4 -left-0 waves z-0', waveColor)}>
          <Wave />
        </div>
        <div className={cn('w-fit h-fit absolute -top-[106px] sm:right-4 -right-0 waves z-0', waveColor)}>
          <Wave />
        </div>
      </>
    )}
    {type === 'crosses' && (
      <>
        <div
          className={cn('w-fit h-fit absolute top-0 -left-10 z-0 animate-[spin_5s_linear_infinite]', waveColor)}
        >
          <Cross />
        </div>
        <div
          className={cn('w-fit h-fit absolute top-1/2 -right-12 z-0 animate-[spin_5s_linear_infinite]', waveColor)}
        >
          <Cross />
        </div>
        <div
          className={cn('w-fit h-fit absolute top-[85%] -left-5 z-0 animate-[spin_5s_linear_infinite]', waveColor)}
        >
          <Cross />
        </div>
      </>
...
export const Heading: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <h1 className={cn('sm:text-4xl leading-[1] text-[clamp(1.7rem,10vw,3rem)] font-black tracking-tight', className)}>
    {children}
  </h1>
)
...
export const Price: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div
    style={{ lineHeight: '1' }}
    className={cn('sm:text-5xl text-[clamp(1.7rem,10vw,3rem)] font-black', className)}
  >
    {children}
  </div>
)
...
export const Paragraph: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div className={cn('sm:text-base text-sm font-medium opacity-90 leading-relaxed', className)}>
    {children}
  </div>
)